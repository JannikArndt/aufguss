...
- **A guess is labelled.** Eight oils have a note the family implied rather than
  Aromen stated; they carry `noteEstimated: true` and the app says so on the
  oil. Any future guess gets the same treatment or does not ship.
...
- **A field the app reads is a field every oil has.** `id`, `de`, `latin`,
  `family`, `familyDe`, and at least one `notes` entry. `tools/smoke.mjs` walks
  all 133 and fails on the first gap.
- **An oil's `family` is one of Aromen's ten scent groups**, verbatim.
  `familyDe` is a translation of the English label and nothing more.
...
- **A theme's `families` is filled only where the plan says it out loud** —
  ...
  name as a hint.
- **A custom oil's id starts `own:`.** That prefix is the only thing separating
  it from a catalogue oil anywhere in the app, and it is what makes regenerating
  `src/data/oils.js` safe.
...
- **Oils** — scrape `/de/shop/category/atherische-ole-einzelole-26` (19 pages,
  `?ppg=` is capped at 20), take one product URL per article code, fetch the
  German and English page of each, read `Duftgruppe`, `Gut Für`, and the note
...
`tools/smoke.mjs` pins two counts on purpose — ten families, eight estimated
notes — so a regeneration that moves either fails until the source file is
brought along.
...
115 checks. It loads the whole app — `src/main.js` and everything under it —
