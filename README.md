...
**Find an oil however you happen to think of it.** 133 single oils from
...
family, the note, the effect tags, or Aromen's article code. `santalum`,
`sandalwood`, `Sandelholz` and `w8` all find the same bottle.
...
**Your own oils and your own notes.** Anything not in the Aromen range gets
added in two taps and lives alongside the catalogue. Every oil, theirs or
yours, carries a note that is yours.
...
Nothing about an oil is written from memory. Every fact was read off a page on
**8 September 2026**, and [`sources/`](sources/) says which page, what it said,
and where the judgement calls are.

| what | from | file |
|---|---|---|
| 133 oils, families, notes, effect tags, descriptions | aromen.be, one product page each | [`sources/oils.md`](sources/oils.md) |
| the botanical names | Wikipedia + Wikidata `P225` | [`sources/botanical-names.md`](sources/botanical-names.md) |
...
- **125 of the 133 notes are Aromen's own words** — their description says
  "Diese luxuriöse Basisnote". The other **eight are estimated from the scent
  family**, and the app says so on the oil rather than pretending.
...
appeared or gone from the Aromen range, whether the Kaifubad PDF has changed,
and whether the four blending pages are still up.
...
node tools/smoke.mjs             # the whole app, in Node, 115 checks
